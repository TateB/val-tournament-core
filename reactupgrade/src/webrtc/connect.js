...
export async function connect() {
    ...
    const neededConnections = ["mapbans", "predictions", "scores", "scores_start", "scores_break", "scores_characterselect", "timer"]
    const findConnection = (protocol) => connections.find(x => x.protocol === protocol)


    function createPeers() {
        for (let connection of neededConnections) {
            connections.push({peer: new rtcConnect(connection), protocol: connection, connected: false })
        }
    }
